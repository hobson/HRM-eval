...
# Hierarchical Reasoning Model Analysis dependencies
sudo apt install python3.12-dev -y

# # Step 1: set up a venv.
# # sudo snap install astral-uv --classic
export TORCH_DEVICE=cpu
rm -rf .venv_$TORCH_DEVICE
uv venv .venv_$TORCH_DEVICE -p 3.12
source .venv_$TORCH_DEVICE/bin/activate
...
uv pip install packaging ninja wheel build setuptools setuptools-scm
